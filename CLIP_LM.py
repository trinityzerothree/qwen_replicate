...
for fs in [1,3,5,10]:
    accuracy = []
    for i in range(1000):
        support_set, sup_labels, query_set, qry_labels = sample_eps(emb, k = fs)
        index = faiss.IndexFlatIP(support_set.shape[1])  #512
    ...
    print("at k = ", fs, "the mean acc is: ", mean(accuracy))